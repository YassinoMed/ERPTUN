{{ Form::model($visitor, ['route' => ['visitors.update', $visitor->id], 'method' => 'PUT', 'class' => 'needs-validation', 'novalidate']) }}
<div class="modal-body">
    <div class="row">
        ...
        <div class="col-md-6"><div class="form-group">{{ Form::label('host_employee_id', __('Host Employee'), ['class' => 'form-label']) }}{{ Form::select('host_employee_id', $employees, $visitor->host_employee_id, ['class' => 'form-control select']) }}</div></div>
        <div class="col-md-3"><div class="form-group">{{ Form::label('visit_date', __('Visit Date'), ['class' => 'form-label']) }}<x-required></x-required>{{ Form::date('visit_date', $visitor->visit_date, ['class' => 'form-control', 'required' => 'required']) }}</div></div>
        <div class="col-md-3"><div class="form-group">{{ Form::label('visit_time', __('Visit Time'), ['class' => 'form-label']) }}{{ Form::time('visit_time', $visitor->visit_time, ['class' => 'form-control']) }}</div></div>
        ...
        <div class="col-md-3"><div class="form-group">{{ Form::label('status', __('Status'), ['class' => 'form-label']) }}<x-required></x-required>{{ Form::select('status', $statuses, $visitor->status, ['class' => 'form-control select', 'required' => 'required']) }}</div></div>
