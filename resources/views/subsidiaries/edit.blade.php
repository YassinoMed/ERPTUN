{{ Form::model($subsidiary, ['route' => ['subsidiaries.update', $subsidiary->id], 'method' => 'PUT', 'class' => 'needs-validation', 'novalidate']) }}
<div class="modal-body">
    <div class="row">
        ...
        <div class="col-md-6"><div class="form-group">{{ Form::label('ownership_percentage', __('Ownership %'), ['class' => 'form-label']) }}{{ Form::number('ownership_percentage', null, ['class' => 'form-control', 'step' => '0.0001']) }}</div></div>
        ...
        <div class="col-md-6"><div class="form-group">{{ Form::label('consolidation_method', __('Consolidation Method'), ['class' => 'form-label']) }}<x-required></x-required>{{ Form::select('consolidation_method', $methods, $subsidiary->consolidation_method, ['class' => 'form-control select', 'required' => 'required']) }}</div></div>
        <div class="col-md-6"><div class="form-group">{{ Form::label('status', __('Status'), ['class' => 'form-label']) }}<x-required></x-required>{{ Form::select('status', $statuses, $subsidiary->status, ['class' => 'form-control select', 'required' => 'required']) }}</div></div>
