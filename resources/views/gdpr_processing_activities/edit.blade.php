{{ Form::model($gdprActivity, ['route' => ['gdpr-activities.update', $gdprActivity->id], 'method' => 'PUT']) }}
...
<div class="col-md-6"><div class="form-group">{{ Form::label('status', __('Status'), ['class'=>'form-label']) }}{{ Form::select('status', $statuses, $gdprActivity->status, ['class'=>'form-control select']) }}</div></div>
