{{ Form::model($configurationItem, ['route' => ['configuration-items.update', $configurationItem->id], 'method' => 'PUT']) }}
...
<div class="col-md-3"><div class="form-group">{{ Form::label('status', __('Status'), ['class'=>'form-label']) }}{{ Form::select('status', $statuses, $configurationItem->status, ['class'=>'form-control select']) }}</div></div>
<div class="col-md-4"><div class="form-group">{{ Form::label('criticality', __('Criticality'), ['class'=>'form-label']) }}{{ Form::select('criticality', $criticalities, $configurationItem->criticality, ['class'=>'form-control select']) }}</div></div>
<div class="col-md-4"><div class="form-group">{{ Form::label('asset_id', __('Linked Asset'), ['class'=>'form-label']) }}{{ Form::select('asset_id', $assets, $configurationItem->asset_id, ['class'=>'form-control select']) }}</div></div>
<div class="col-md-4"><div class="form-group">{{ Form::label('owner_user_id', __('Owner'), ['class'=>'form-label']) }}{{ Form::select('owner_user_id', $users, $configurationItem->owner_user_id, ['class'=>'form-control select']) }}</div></div>
...
<div class="col-md-4"><div class="form-group">{{ Form::label('acquired_at', __('Acquired At'), ['class'=>'form-label']) }}{{ Form::date('acquired_at', optional($configurationItem->acquired_at)->format('Y-m-d'), ['class'=>'form-control']) }}</div></div>
