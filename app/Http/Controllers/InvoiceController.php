...
//                $invoiceProduct->discount    = isset($products[$i]['discount']) ? $products[$i]['discount'] : 0;
    ...
    public function update(Request $request, Invoice $invoice)
    {
        ...
        if (\Auth::user()->can('edit invoice')) {
            if ($invoice->created_by == \Auth::user()->creatorId()) {
                ...
                $invoice->save();
                if (! $wasPaid && (int) $invoice->status === 4) {
                    Utility::dispatchAutomationEvent('invoice.paid', $invoice, \Auth::user()->creatorId());
                }
    ...
    public function bulk(Request $request)
    {
        ...
        if ($action === 'export') {
            ...
            $fileName = 'invoices-' . date('Ymd-His') . '.csv';

            return response()->streamDownload(function () use ($invoices) {
                $out = fopen('php://output', 'w');
                fputcsv($out, ['ID', 'Invoice', 'Customer', 'Status', 'Issue Date', 'Due Date', 'Total']);
                foreach ($invoices as $invoice) {
                    $customerName = $invoice->customer ? $invoice->customer->name : '';
                    $statusLabel = isset(Invoice::$statues[$invoice->status]) ? Invoice::$statues[$invoice->status] : $invoice->status;
                    fputcsv($out, [
                        $invoice->id,
        ...
        if ($action === 'delete') {
            if (!$request->isMethod('post')) {
                return response()->json(['ok' => false, 'message' => __('Method not allowed.')], 405);
            }
            if (!\Auth::user()->can('delete invoice')) {
            ...
            foreach ($ids as $id) {
                ...
                if (!$invoice) {
                    $failed[] = ['id' => $id, 'message' => __('Invoice Not Found.')];
                    continue;
                }

                $customerCreditNote = CustomerCreditNotes::where('invoice', $invoice->id)->first();
                if ($customerCreditNote) {
                    $failed[] = ['id' => $id, 'message' => __('The credit note has been created for this invoice, so it cannot be deleted.')];
                    continue;
                }

                try {
                    foreach ($invoice->payments as $invoicesPayment) {
                        Utility::bankAccountBalance($invoicesPayment->account_id, $invoicesPayment->amount, 'debit');
                        Utility::updateUserBalance('customer', $invoice->customer_id, $invoicesPayment->amount, 'credit');

                        $invoicepayment = InvoicePayment::find($invoicesPayment->id);
                        if ($invoicepayment) {
                            AddTransactionLine::where('reference_id', $invoice->id)
                                ->where('reference_sub_id', $invoicepayment->id)
                                ->where('reference', 'Invoice Payment')
                                ->delete();
                    ...
                    $failed[] = ['id' => $id, 'message' => __('Unable to delete this invoice.')];
                }
            }

            return response()->json(['ok' => true, 'deleted' => $deleted, 'failed' => $failed]);
        }

        return response()->json(['ok' => false, 'message' => __('Invalid action.')], 422);
    }
    ...
    public function savedFilters(Request $request)
    {
        ...
        $userId = (int) \Auth::id();
        $filters = $this->getInvoiceSavedFilters($userId);

        $payload = array_map(function ($filter) {
            $id = $filter['id'] ?? null;
            $name = $filter['name'] ?? '';
            ...
            return [
                'id' => $id,
                'name' => $name,
                'apply_url' => $id ? route('invoice.filters.apply', $id) : null,
                'delete_url' => $id ? route('invoice.filters.delete', $id) : null,
            ];
        }, $filters);

        return response()->json(['filters' => $payload]);
    }
    ...
    public function saveFilter(Request $request)
    {
        ...
        if ($validator->fails()) {
            return response()->json(['flag' => 0, 'msg' => $validator->errors()->first()], 422);
        }
        ...
        if (!$hasAny) {
            return response()->json(['flag' => 0, 'msg' => __('Please select at least one filter.')], 422);
        }
        ...
        foreach ($filters as $index => $filter) {
            if (isset($filter['name']) && mb_strtolower((string) $filter['name']) === mb_strtolower($name)) {
                $existingIndex = $index;
                break;
            }
        }
        ...
        $this->putInvoiceSavedFilters($userId, $filters);

        return response()->json(['flag' => 1, 'msg' => __('Saved.')]);
    }
    ...
    public function deleteFilter($id)
    {
        ...
        $userId = (int) \Auth::id();
        $filters = $this->getInvoiceSavedFilters($userId);
        $filters = array_values(array_filter($filters, function ($filter) use ($id) {
            return isset($filter['id']) && (string) $filter['id'] !== (string) $id;
        }));

        $this->putInvoiceSavedFilters($userId, $filters);

        return response()->json(['flag' => 1, 'msg' => __('Deleted.')]);
    }
    ...
    public function applyFilter($id)
    {
        ...
        foreach ($filters as $filter) {
            if (isset($filter['id']) && (string) $filter['id'] === (string) $id) {
                $match = $filter;
                break;
            }
        }

        if (!$match || !isset($match['query']) || !is_array($match['query'])) {
            return redirect()->route('invoice.index')->with('error', __('Filter not found.'));
        }

        return redirect()->route('invoice.index', $match['query']);
    }

    private function invoiceSavedFiltersCacheKey(int $userId): string
    {
        return 'invoice.saved_filters.user.' . $userId;
    }

    private function getInvoiceSavedFilters(int $userId): array
    {
        $filters = cache()->get($this->invoiceSavedFiltersCacheKey($userId), []);

        if (!is_array($filters)) {
            return [];
        }

        return array_values(array_filter($filters, function ($filter) {
            return is_array($filter) && isset($filter['id'], $filter['name'], $filter['query']) && is_array($filter['query']);
        }));
    }

    private function putInvoiceSavedFilters(int $userId, array $filters): void
    {
        cache()->put($this->invoiceSavedFiltersCacheKey($userId), $filters, now()->addDays(365));
    }
